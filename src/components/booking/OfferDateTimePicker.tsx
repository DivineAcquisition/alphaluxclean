...
interface OfferDateTimePickerProps {
  ...
  /**
   * Earliest bookable day, counted as "today + N days". AlphaLuxClean
   * requires at least 3 days of lead time so the ops team can schedule
   * a crew.
   */
  minLeadDays?: number;
}
 ...
 * familiar month-grid interface instead of a horizontal date-strip.
 *
 * Intentionally uses the project's semantic color tokens
 * (`primary`, `border`, `muted-foreground`) rather than a brand-specific
 * palette, so this component inherits AlphaLuxClean's existing
 * navy/slate theme without any per-repo color constants.
 */
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-[1.15fr_1fr] md:gap-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge
              variant="outline"
              className="border-primary/40 text-primary text-[10px] font-semibold uppercase tracking-wider"
            >
              {leadDaysLabel}
            </Badge>
          </div>
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                variant="outline"
                className={cn(
                  'w-full justify-start rounded-xl border-2 border-border bg-card px-4 py-6 text-left hover:border-primary/60',
      ...
      {selectedDateLabel && timeSlot && (
        <>
          <Separator />
          <div className="flex items-start gap-3 rounded-xl bg-primary/10 border border-primary/25 p-3 text-sm">
            <Check className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
            <div>
              <p className="font-semibold text-foreground">Slot reserved</p>
