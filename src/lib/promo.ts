/**
 * AlphaLuxClean (TX / CA) — Promotional code constants.
 *
 * Mirrors the promo surface used by the New York repo (alphaluxcleaning)
 * so the Offer step, checkout summary and confirmation screen share a
 * single promo vocabulary across both storefronts. Both repos point at
 * the same Supabase project, so the code `ALC2026` is validated and
 * redeemed server-side by the shared `promo-system` edge function.
 *
...
export const NEW_CUSTOMER_PROMO_DESCRIPTION = NEW_CUSTOMER_PROMO_ACTIVE
  ? `New customers save ${NEW_CUSTOMER_PROMO_PERCENT}% on their first AlphaLuxClean with code ${NEW_CUSTOMER_PROMO_CODE}.`
  : "";
